...
export function validateSegmentData(data) {
  ...
  // Check if we have either the expected format or the actual LLM format
  
  // Check for LLM actual format (growth objects)
  ...
  if (hasGrowthSegments && hasOverallPerformance) {
    return true; // LLM format is valid
  }
  
  // Check for original expected format (segments array)
  if (Array.isArray(data.segments) && data.segments.length > 0 && data.overall) {
