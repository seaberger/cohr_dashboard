...
export const FRONTEND_TILE_MAPPINGS = {
  ...
  // Legacy structure mappings (backward compatibility)
  'networking': ['networkingGrowth', 'aiDatacomGrowth'],
  'telecom': ['telecomGrowth'],
  'laser': ['industrialLaserMarket'], 
  'materials': ['materialsGrowth']
};

export function identifySegmentType(segmentName) {
  const name = segmentName.toLowerCase();
  
  // Check for exact new structure matches first (higher priority)
  if (name.includes('datacenter') && name.includes('communication')) {
    return 'datacenterComms';
  }
  if (name.includes('industrial') && !name.includes('laser')) {
    // "Industrial" as standalone segment (new structure)
    return 'industrial';
  }
  
  // Check all mapping keywords
